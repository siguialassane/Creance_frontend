...
export function useCompteOperations() {
...
export function useSearchCompteOperations(searchTerm: string) {
  const apiClient = useApiClient();
  const { data: session, status } = useSession();
...
export function useCreateCompteOperation() {
  ...
  return useMutation({
    mutationFn: (compteOperation: CompteOperationCreateRequest) => CompteOperationService.create(apiClient, compteOperation),
    onSuccess: (data) => {
...
export function useUpdateCompteOperation() {
  ...
  return useMutation({
    mutationFn: ({ code, compteOperation }: { code: string; compteOperation: CompteOperationUpdateRequest }) =>
      CompteOperationService.update(apiClient, code, compteOperation),
