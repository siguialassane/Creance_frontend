...
export function useEtapes() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await EtapeService.getAll(apiClient);
        console.log('📦 Réponse brute etapes:', res);
        ...
        console.log('✅ Données etapes transformées:', data);
        return Array.isArray(data) ? data : [];
      } catch (error) {
        console.error('❌ Erreur chargement etapes:', error);
        return [];
      }
    },
