...
/**
 * Hook pour charger les entités avec recherche et pagination infinie
 */
export function useEntitesSearchable() {
  ...
  const {
    ...
    queryKey: ["entites", "searchable", search],
    queryFn: async ({ pageParam = 0 }) => {
      try {
        ...
        try {
          const response = await fetchPaginatedData<any>("/entites", params)
          ...
          const response = await EntiteService.getAll(apiClient as any)
