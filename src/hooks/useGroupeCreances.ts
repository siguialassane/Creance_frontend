...
export function useGroupeCreances() {
...
export function useSearchGroupeCreances(searchTerm: string) {
  const apiClient = useApiClient();
  const { data: session, status } = useSessionWrapper();
...
export function useCreateGroupeCreance() {
  ...
  return useMutation({
    mutationFn: (groupeCreance: GroupeCreanceCreateRequest) => GroupeCreanceService.create(apiClient, groupeCreance),
    onSuccess: (data) => {
...
export function useUpdateGroupeCreance() {
  ...
  return useMutation({
    mutationFn: ({ code, groupeCreance }: { code: string; groupeCreance: GroupeCreanceUpdateRequest }) =>
      GroupeCreanceService.update(apiClient, code, groupeCreance),
