...
export class ObjetCreanceService {
  ...
  static async getByCode(apiClient: any, code: string): Promise<ObjetCreance> {
    const response = await apiClient.get<ObjetCreanceApiResponse>(`${ObjetCreanceService.BASE_URL}/${code}`);
  ...
  static async create(apiClient: any, objet: ObjetCreanceCreateRequest): Promise<ObjetCreanceApiResponse> {
    const response = await apiClient.post<ObjetCreanceApiResponse>(ObjetCreanceService.BASE_URL, objet);
    return response.data;
  }

  static async update(apiClient: any, code: string, objet: ObjetCreanceUpdateRequest): Promise<ObjetCreanceApiResponse> {
    const response = await apiClient.put<ObjetCreanceApiResponse>(`${ObjetCreanceService.BASE_URL}/${code}`, objet);
    return response.data;
  }

  static async delete(apiClient: any, code: string): Promise<ObjetCreanceApiResponse> {
    const response = await apiClient.delete<ObjetCreanceApiResponse>(`${ObjetCreanceService.BASE_URL}/${code}`);
    return response.data;
  }

  static async search(apiClient: any, searchTerm: string): Promise<ObjetCreanceApiResponse> {
    const response = await apiClient.get<ObjetCreanceApiResponse>(`${ObjetCreanceService.BASE_URL}/search`, {
      params: { q: searchTerm }
    });
    return response.data;
  }
