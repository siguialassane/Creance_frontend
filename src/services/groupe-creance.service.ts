...
export class GroupeCreanceService {
  ...
  static async getByCode(apiClient: any, code: string): Promise<GroupeCreance> {
    const response = await apiClient.get<GroupeCreanceApiResponse>(`${GroupeCreanceService.BASE_URL}/${code}`);
  ...
  static async create(apiClient: any, groupe: GroupeCreanceCreateRequest): Promise<GroupeCreanceApiResponse> {
    const response = await apiClient.post<GroupeCreanceApiResponse>(GroupeCreanceService.BASE_URL, groupe);
    return response.data;
  }

  static async update(apiClient: any, code: string, groupe: GroupeCreanceUpdateRequest): Promise<GroupeCreanceApiResponse> {
    const response = await apiClient.put<GroupeCreanceApiResponse>(`${GroupeCreanceService.BASE_URL}/${code}`, groupe);
    return response.data;
  }

  static async delete(apiClient: any, code: string): Promise<GroupeCreanceApiResponse> {
    const response = await apiClient.delete<GroupeCreanceApiResponse>(`${GroupeCreanceService.BASE_URL}/${code}`);
    return response.data;
  }

  static async search(apiClient: any, searchTerm: string): Promise<GroupeCreanceApiResponse> {
    const response = await apiClient.get<GroupeCreanceApiResponse>(`${GroupeCreanceService.BASE_URL}/search`, {
      params: { q: searchTerm }
    });
    return response.data;
  }
