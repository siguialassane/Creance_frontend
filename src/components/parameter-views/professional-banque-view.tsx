...
const columns = [
  {
    key: 'code' as keyof Banque,
    label: 'Code',
    sortable: true,
  },
  {
    key: 'libelle' as keyof Banque,
    label: 'Libellé',
    sortable: true,
  },
  {
    key: 'responsable' as keyof Banque,
    label: 'Responsable',
    sortable: true,
  },
  {
    key: 'adresse' as keyof Banque,
    label: 'Adresse',
    sortable: false,
  }
]

export default function ProfessionalBanqueView() {
  ...
  const handleView = (banque: Banque) => {
    alert(`Affichage des détails de : ${banque.libelle}`)
  }
  ...
  return (
    <>
      <ParameterPage
        title="Banques"
        description="Gestion des établissements bancaires partenaires"
        data={data}
        columns={columns}
        searchKey="libelle"
